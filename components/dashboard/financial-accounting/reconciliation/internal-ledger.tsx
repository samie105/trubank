...
export default function SLReconciliationTable() {
  const [sorting, setSorting] = useState<SortingState>([]);
  const [columnFilters, setColumnFilters] = useState<ColumnFiltersState>([]);
  const [columnVisibility, setColumnVisibility] = useState<VisibilityState>({});
  const [rowSelection, setRowSelection] = useState({});
  const [selectedSL, setSelectedSL] = useState<Account | null>(
  ...
  const SLColumn: ColumnDef<Transaction>[] = [
    {
      id: "reference",
  ...
  // Prepare CSV data
  const csvData = SL.map((row) => {
    const csvRow: Record<string, string> = {};
    SLColumn.forEach((col) => {
      if ("accessorKey" in col && typeof col.accessorKey === "string") {
  ...
  const handleSLSelection = (account: Account) => {
    setSelectedSL(account);
    setSLOpen(false);
    toast.success("SL account selected", {
      description: `${account.name} (${account.number}) has been selected.`,
    });
    // Here you would typically fetch or filter data based on the selected GL
    // For demonstration, let's just log it
    console.log("Selected SL:", account);
  };
    ...
    <div className="w-full">
          ...
          <Popover open={sLOpen} onOpenChange={setSLOpen}>
            <PopoverTrigger asChild>
              <Button className="bg-primary hover:bg-primary flex items-center gap-2">
                <span className="hidden md:block">Select SL</span>
                <Plus className="h-4 w-4" />
              </Button>
            </PopoverTrigger>
            <PopoverContent className="w-[250px] p-0">
              <div className="p-4 font-medium">Select Subsidiary Ledger</div>
              <Command>
                  ...
                  <CommandGroup>
                    {glAccounts.map((account) => (
                      <CommandItem
                        key={account.id}
                        value={account.id}
                        onSelect={() => handleSLSelection(account)}
                      >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div className=" p-4 rounded-lg border">
          <h3 className="font-medium mb-2">Sub Ledger</h3>
      ...
      <div className="flex items-center justify-between py-4">
        <div className="relative w-72 mr-2">
          ...
          <Input
            placeholder="Search"
            value={
              (sl.getColumn("reference")?.getFilterValue() as string) ?? ""
            }
            onChange={(event) =>
              sl.getColumn("reference")?.setFilterValue(event.target.value)
            }
            className="pl-8"
          />
        </div>
      ...
      <div className="mb-6">
        <h3 className="font-medium mb-2">
          ...
          {selectedSL ? selectedSL.number : "2903090290"})
        </h3>
        {selectedSL && (
          <div className="my-2 text-sm text-muted-foreground">
            Selected: {selectedSL.name} ({selectedSL.number})
          </div>
        )}
        <div className="rounded-md border">
          <Table>
            <TableHeader>
              {sl.getHeaderGroups().map((headerGroup) => (
            ...
            <TableBody>
              {sl.getRowModel().rows?.length ? (
                sl.getRowModel().rows.map((row) => (
                  <TableRow
                    key={row.id}
      ...
      <div>
        <h3 className="font-medium mb-2">
          General Ledger - Cash ({selectedGL ? selectedGL.number : "2903090290"}
          )
        </h3>
        {selectedGL && (
          <div className="my-2 text-sm text-muted-foreground">
