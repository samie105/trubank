...
const menuItems = [
  {
    name: "Overview",
    icon: <Element3 size={20} color="#EC9006" />,
    path: "/dashboard/overview",
  },
  {
    name: "Branch Management",
    icon: <Bank size={20} color="#EC9006" />,
...
export default function MobileNav() {
  ...
  const getParentNav = () => {
    ...
    if (parts.length > 2) {
      let parentNav = parts[2]
        .split("-")
        .map((word) => word.charAt(0).toUpperCase() + word.slice(1))
        .join(" ");
      if (parentNav.includes("Overview")) {
        parentNav = "Dashboard Overview";
      }
      return parentNav;
    }
  ...
  return (
    <div className="flex items-center gap-x-2">
                  ...
                  {[
                    ...
                    <SidebarMenuItem key={item.name}>
                      ...
                      >
                        <Link
                          href={item.path}
                          className="text-foreground py-5 flex"
                        >
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
      ...
      <div className="font-semibold text-lg route-name">{getParentNav()}</div>
    </div>
  );
}
