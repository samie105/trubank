...
function BusinessEditor({ params }: { params: { id: string } }) {
    ...
    async function fetchBusinessData() {
      try {
        ...
        const response = await fetch(
          `${apiUrl}/customermanagement/get-business-user?UserId=${params.id}`,
          {
            method: "GET",
            headers: {
    ...
    <div className="container mx-auto py-6">
      ...
      >
        ...
        <TabsContent value="confirm">
          <BusinessConfirmPage isEditMode={true} customerId={params.id} />
        </TabsContent>
      </Tabs>
    </div>
  )
...
export default function EditBusinessCustomerPage({ params }: { params: { id: string } }) {
  return (
    <BusinessFormProvider>
      <BusinessEditor params={params} />
    </BusinessFormProvider>
  )
} 